                ...
                .filter(document -> {
                    if (request.getContainsContents() != null && !request.getContainsContents().isEmpty()) {
                        return request.getContainsContents().stream()
                                      .anyMatch(contents -> document.getContent() != null
                                              && document.getContent().contains(contents));
                    }
                    return true;
                })
                .filter(document -> {
                    if (request.getAuthorIds() != null && !request.getAuthorIds().isEmpty()) {
                        return request.getAuthorIds().stream()
                                .anyMatch(authorId -> document.getAuthor() != null
                                        && document.getAuthor().getId() != null
                                        && document.getAuthor().getId().equals(authorId));
                    }
                    return true;
                })
                .filter(document -> {
